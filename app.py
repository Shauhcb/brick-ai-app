...
def smart_search(query):
    ...
    result = query_ai(prompt)    # 🔒 SYNTAX FIX: Ensure return is on its own line
    ...
    try:
        ...
        for title in results:
            try:
                ...
                summary = wikipedia.summary(title, sentences=3)                summaries.append({
                    'title': title,
                    'summary': summary,
                    'url': page.url
                })
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .chat-send-btn {
            ...
            font-weight: bold;
            cursor: pointer;
            transition: all 0.3s;            box-shadow: 0 0 20px rgba(0,255,65,0.3);
        }
        .chat-send-btn:hover { 
            transform: translateY(-2px); 
            box-shadow: 0 0 40px rgba(0,255,65,0.5);
        ...
        .history-item:hover { background: #e8eaff; }        .history-item strong { color: #333; }
...
<body>
        ...
        <div id="searchTab" class="tab-content active">
            <div class="search-box">
                ...
                <div class="search-mode">
                    <button class="mode-btn active" onclick="setMode('all', this)">🔍 All</button>
                    <button class="mode-btn" onclick="setMode('google', this)">🌐 Google</button>
                    <button class="mode-btn" onclick="setMode('bing', this)">🔎 Bing</button>
                    <button class="mode-btn" onclick="setMode('wiki', this)">📚 Wiki</button>
                    <button class="mode-btn" onclick="setMode('ai', this)">🤖 AI</button>                </div>
        ...
        <div id="chatTab" class="tab-content">
            <div class="chat-container">
                ...
                <div class="chat-input-area">
                    ...
                    <button class="chat-send-btn" id="chatSendBtn" onclick="sendMessage()">Send</button>                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function clearHistory() {            if(confirm('Are you sure you want to clear your search history?')) {
        ...
        function addMessage(type, text) {
            const container = document.getElementById('chatMessages');            const time = new Date().toLocaleTimeString();
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .links a:hover { text-decoration: underline; color: #00ff41; }    </style>
</head>
<body>
    <div class="login-box">
...
def search():
    ...
    c = conn.cursor()
    history = c.execute('SELECT * FROM search_history WHERE user_id = ? ORDER BY timestamp DESC LIMIT 10',
                       (session['user_id'],)).fetchall()    conn.close()
